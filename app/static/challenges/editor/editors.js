...
const DECOMP_HEADER = [
  "; This is the decompiling results of SOTA decompiler.",
  "; You should recover the source code based on this result.",
  "; Try to make it re-compile successfully and generate the given ASM.",
  "",
  ""
].join('\n')
...
const editors = {
  ...
  decompile : function(name) {
    const info = decompile[name]
    return (info)? info.asm : ''
  },
  ...
  update: function() {
    ...
    if(mode === 'target') {
      ...
      if(!decompile[func]) {
        ...
        let text = decompile[func].asm
        if(repl) text = editors.replace(text, 'decompile')
        editors.showText(DECOMP_HEADER + text)
      }
    }
